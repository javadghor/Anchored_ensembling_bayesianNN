...
#### Added late
def plot_yerr(ytest, ypred, yerr, type_err, bounds_outputs, output_names, ax_lims, marker='+'):
    fig, ax = plt.subplots(ncols=5, figsize=(15, 3.))
    for i, (y_true, y_pm, y_ps) in enumerate(zip(ytest.T, ypred.T, yerr.T)):
        bds_i = bounds_outputs[i]
        ax[i].plot(ax_lims, ax_lims, color='gray', linestyle='--')
        if type_err == 'std':
            ax[i].errorbar(
                x=y_true, y=y_pm, yerr=2*y_ps, color='firebrick', linestyle='none', marker=marker, label='aleatoric uncertainty')
        elif type_err == 'perc':
            y_ps[..., 0] = y_pm-y_ps[..., 0]
            y_ps[..., 1] = y_ps[..., 1]-y_pm
            ax[i].errorbar(
                x=y_true, y=y_pm, yerr=y_ps.T, color='firebrick', linestyle='none', marker=marker, label='aleatoric uncertainty')
        ax[i].set_xticks([-1, 0, 1])
        ax[i].set_xticklabels(['{:.3g}'.format(val) for val in [bds_i[0], (bds_i[0]+bds_i[1])/2, bds_i[1]]], fontsize=10)
        ax[i].set_xlabel('$y_{true}$', fontsize=14)
        ax[i].set_yticks([-1, 0, 1])
        ax[i].set_yticklabels(['{:.3g}'.format(val) for val in [bds_i[0], (bds_i[0]+bds_i[1])/2, bds_i[1]]], fontsize=10)
        if i == 0:
            ax[i].set_ylabel('$y_{predicted}$', fontsize=14)
        ax[i].set_title(output_names[i], fontsize=16)
        ax[i].set_xlim(ax_lims)
...
def plot_law_MC(a_fixed, ytest, yMC, indices_test, yMCaleatoric=None, plot_mean_law=False):
    fig, ax = plt.subplots(ncols=len(indices_test), figsize=(14, 2.5))
    domain = np.linspace(0.0, 0.05, 100)
    for n, (i, c) in enumerate(zip(indices_test, colors_data)):
        mean_law = np.zeros((len(domain), ))
        percentiles_total = np.zeros((len(domain), 2))
        percentiles_aleatoric = np.zeros((len(domain), 2))
        yMC_for_mean = yMC[yMC[:, i, 1] > 0., i, :]   # remove points for which b or c is negative
        for d_, d in enumerate(domain):
            law_MC = a_fixed + yMC[:, i, 0] * d ** yMC[:, i, 1]
            percentiles_total[d_, :] = np.percentile(law_MC, [2.5, 97.5])
            mean_law[d_] = np.mean(a_fixed + yMC_for_mean[:, 0] * d ** yMC_for_mean[:, 1])
        ...
        if plot_mean_law:
            ax[n].plot(domain, mean_law, color='gray', linestyle='--', label='mean prediction')
        if yMCaleatoric is not None:
